                    ...
                    <View pb={15} paddingHorizontal={15} pt={15} $xl={{ minWidth: '40%' }} $sm={{ minWidth: '90%' }} backgroundColor='white' borderRadius={10} justifyContent='center'>
                        ...
                        {(!step3 && !step4) && (
                            <>
                                ...
                                <Input mt={15} mb={15} onChangeText={setEmailModal} placeholder='E-mail' value={emailModal} focusStyle={{ borderColor: '#049A63', borderWidth: 1 }}
                                    hoverStyle={{ borderColor: '#049A63', borderWidth: 1 }}></Input>
                                {step2 && <Input onChangeText={setCodeModal} maxLength={5} placeholder='Código' value={codeModal} focusStyle={{ borderColor: '#049A63', borderWidth: 1 }}
                                    hoverStyle={{ borderColor: '#049A63', borderWidth: 1 }}></Input>}
                        ...
                        {
                            (step3 && !step4) && (
                                <Input mt={15} mb={15} onChangeText={setPasswordModal} placeholder='Nova senha' value={passwordModal} focusStyle={{ borderColor: '#049A63', borderWidth: 1 }}
                                    hoverStyle={{ borderColor: '#049A63', borderWidth: 1 }}></Input>
                            )
                        }
                        {!step4 && (
                                ...
                                <Button onPress={async () => {
                                    if (step3) {
                                        const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/auth/pwChange`, {
                                            method: 'POST',
                                            body: JSON.stringify({ email: emailModal.toLowerCase(), codeSent: codeModal, newPW: passwordModal }),
                                            headers: {
                                        ...
                                        if (!step2) {
                                            ...
                                            const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/auth/recoveryCheck`, {
                                                method: 'POST',
                                                body: JSON.stringify({ email: emailModal.toLowerCase(), codeSent: codeModal }),
                                                headers: {
                                                    'Content-Type': 'application/json'
                                                }
                                            })
    ...
    const login = async (email: string, password: string, registerInvalid: Function, setErros: Function) => {
        if (email.length && password.length && email.length <= 256 && password.length <= 35) {
            ...
            try {
                ...
                const res: { data: { token: string, role: string[] }, status: number, msg: string | null } = await response.json()
                if (response.ok) {
    ...
    const register = async (email: string, emailValid: boolean, password: string, passwordValid: boolean, registerInvalid: Function, setErros: Function) => {
        ...
        if (email.length > 1 && emailValid && password.length >= 8 && passwordValid) {
            ...
            try {
                ...
                if (response.ok) {
                    const res: { data: { token: string, role: string[] }, status: number } = await response.json()
                    await Promise.all([setToken(res.data.token), AsyncStorage.setItem('role', res.data.role[0])])
                    if (res.data.role.includes('registering')) {
                        props.navigation.replace('Register')
                    } else {
...
export function SignInWeb(props: { page: string, onButtonPress: (page: string) => void, navigation: NativeStackNavigationProp<RootStackParamList, "Home">, modal: () => void }) {
    ...
    return (
        ...
        <YStack px={24} f={1} justifyContent='center' alignItems='center'>
            ...
            <Button onPress={async () => { await login(email, password, setRegisterInvalid, setErros) }} hoverStyle={{ backgroundColor: '#03a86c' }} mt='$3.5' backgroundColor='#04BF7B' color='white' fontWeight='$10' width='$20'>Entrar</Button>
        ...
        <YStack px={24} f={1} justifyContent='center' alignItems='center'>
            ...
            <XStack {...emailValid ? { borderColor: 'lightgray' } : { borderColor: '$red10' }} width='$20' backgroundColor='white' borderWidth={1} borderRadius={9} borderColor='lightgray' mt='$3.5' alignItems='center' flexDirection='row' zIndex={20}>
                <Input placeholder='Email' onChangeText={(email) => {
                    setEmail(email); emailIsValid(email, setEmailValid)
                }} focusStyle={{ outlineStyle: 'none' }} backgroundColor='$colorTransparent' borderWidth='0' f={1} maxLength={256} width='100%' />
            </XStack>
            <XStack width='$20' backgroundColor='white' pr='$3.5' borderWidth={1} borderRadius={9} borderColor='lightgray' mt='$3.5' alignItems='center' flexDirection='row' zIndex={20}>
                <Input onChangeText={(e) => {
                    setPassword(e)
                    passwordIsValid(e, confirmPassword, setPasswordValid)
                }} focusStyle={{ outlineStyle: 'none' }} placeholder='Senha' backgroundColor='$colorTransparent' borderWidth='$0' borderColor='$colorTransparent' secureTextEntry={showPw} f={1} mr='$3.5' maxLength={20} />
                <Icons name={showPw ? 'eye' : 'eye-off'} size={24} onPress={() => { setShowPw(!showPw) }}></Icons>
            </XStack>
            <XStack width='$20' backgroundColor='white' pr='$3.5' borderWidth={1} borderRadius={9} borderColor='lightgray' mt='$3.5' alignItems='center' flexDirection='row' zIndex={20}>
                <Input onChangeText={(e) => {
                    setConfirmPassword(e)
                    passwordIsValid(e, password, setPasswordValid)
                }} focusStyle={{ outlineStyle: 'none' }} placeholder='Confirmar senha' backgroundColor='$colorTransparent' borderWidth='$0' borderColor='$colorTransparent' secureTextEntry={showConfirmPw} f={1} mr='$3.5' maxLength={20} />
                <Icons name={showConfirmPw ? 'eye' : 'eye-off'} size={24} onPress={() => { setShowConfirmPw(!showConfirmPw) }}></Icons>
            </XStack>

            <Button onPress={async () => {
                await register(email, emailValid, password, passwordValid, setRegisterInvalid, setErros)
            }} hoverStyle={{ backgroundColor: '#03a86c' }} mt='$3.5' backgroundColor='#04BF7B' color='white' fontWeight='$10' width='$20'>Cadastrar</Button>
