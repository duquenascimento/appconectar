...
export function SupplierProvider({ children }: { children?: ReactNode }) {
  ...
  const saveSuppliersToStorage = async (
    available: SupplierData[],
    unavailable: SupplierData[]
  ) => {
    try {
  ...
  const loadPrices = useCallback(async (restaurantId?: string) => {
    try {
      ...
      const currentRestaurant = allRestaurants.find(
        (r: any) =>
          r.externalId === (restaurantId ?? restaurantSelected?.externalId)
      )
      ...
      if (
        currentRestaurant?.allowClosedSupplier &&
        currentRestaurant?.allowMinimumOrder
      ) {
        available = allSuppliers.filter(
          (item) => item.supplier.missingItens > 0
        )
      } else {
        available = allSuppliers.filter(
          (item) =>
