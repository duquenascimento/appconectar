...
export function PixDisplay(props: { pixCharge?: PixCharge | null }) {
  ...
  return (
    <View maxWidth={200} gap={'$2'}>
      <img
        src={pixCharge ? `data:image/png;base64,${pixCharge?.encodedImage}` : undefined}
        alt="QR Code"
      />
      {pixCharge && (
        <Text color="gray" fontSize={10} textAlign="center">
          {' '}
          {addMinutes(30, pixCharge!.createdAt).toFormat("'Válido até' dd/MM/yyyy HH:mm")}{' '}
        </Text>
      )}
      <Button
