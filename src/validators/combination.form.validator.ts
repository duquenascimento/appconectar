...
export const combinacaoValidationSchema = Yup.object({
  ...
  dividir_em_maximo: Yup.number()
    .integer()
    .min(2, 'Selecione o máximo de fornecedores.')
    .required(),
  ...
  fornecedores_bloqueados: Yup.array(Yup.string()).when('bloquear_fornecedores', {
    ...
    otherwise: (schema) =>
      schema.max(0, 'Remova os fornecedores bloqueados se não for usar a opção.'),
  }),

  fornecedores_especificos: Yup.array(Yup.string()).test(
    'min-fornecedores-especificos',
    'Selecione fornecedores suficientes.',
      ...
      if (preferencia_fornecedor_tipo === 'especifico') {
        ...
        if (minSuppliers < 2) {
          return this.createError({
            message: 'Selecione a quantidade de fornecedores para dividir.',
          });
        }
        if (!value || value.length < minSuppliers) {
          return this.createError({
            message: `Selecione pelo menos ${minSuppliers} fornecedor(es) específico(s).`,
          });
        }
      }
  ...
  preferencias: Yup.array(preferenciaProdutoSchema).when(
    ['definir_preferencia_produto', 'fornecedores_especificos', 'preferencia_fornecedor_tipo'],
    {
      is: (definir: boolean, fornecedores: string[], tipo: string) =>
        definir && (tipo === 'especifico' ? fornecedores.length > 0 : true),
      then: (schema) => schema.min(1, 'Adicione ao menos uma preferência de produto.'),
      otherwise: (schema) =>
        schema.max(
          0,
          'Remova as preferências se não estiver definindo ou não houver fornecedores.',
        ),
    },
  ),
});
